...
export function renderProfileCardHtml(profile: SearchableProfile): string {
  ...
  const tagPills = (profile.tags || [])
    .map(
      (tag) =>
        `<a href="/directorio/?q=${encodeURIComponent(tag)}" class="${profileTagPillClass}">${escapeHtml(tag)}</a>`,
    )
    .join("");
  ...
  const facetRow = chips || tagPills
    ? `<div class="mt-auto min-h-12 shrink-0 border-t border-dm-offblack/10 pt-3"><div class="flex flex-wrap content-start gap-1.5">${chips}</div>${tagsRow}</div>`
